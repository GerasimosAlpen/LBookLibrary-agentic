...
<div class="min-h-screen flex items-center justify-center px-4 py-12">
    <div class="w-full max-w-md">
        ...
        <div class="text-center mb-8">
            ...
            <p class="mt-1 text-sm text-slate-400">Join the BookLib library today</p>
        </div>
        ...
        <div class="bg-slate-800/60 backdrop-blur-xl border border-slate-700/50 rounded-2xl shadow-2xl shadow-black/40 p-8">
            ...
            {{-- Global Errors --}}
            @if ($errors->any())
                <div id="error-banner" class="mb-6 flex gap-3 items-start bg-red-500/10 border border-red-500/30 rounded-xl px-4 py-3 text-sm text-red-300 animate-fade-in">
            ...
            <form id="register-form" action="{{ route('auth.register') }}" method="POST" novalidate>
                @csrf
                ...
                <div class="mb-5">
                    ...
                    <input
                        ...
                        class="w-full px-4 py-2.5 bg-slate-900/60 border @error('name') border-red-500/60 @else border-slate-600/60 @enderror rounded-xl text-sm text-slate-100 placeholder-slate-500
                ...
                <div class="mb-5">
                    ...
                    <div class="relative">
                        <input
                            ...
                            class="w-full px-4 py-2.5 pr-11 bg-slate-900/60 border @error('password') border-red-500/60 @else border-slate-600/60 @enderror rounded-xl text-sm text-slate-100 placeholder-slate-500
                                   focus:outline-none focus:ring-2 focus:ring-indigo-500/60 focus:border-indigo-500/60
                                   transition duration-200"
                        />
                        <button type="button" onclick="togglePwd('password', 'eye-pwd')" class="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-200 transition">
                            <svg id="eye-pwd" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    ...
                    <div class="relative">
                        <input
                            ...
                            class="w-full px-4 py-2.5 pr-11 bg-slate-900/60 border border-slate-600/60 rounded-xl text-sm text-slate-100 placeholder-slate-500
                                   ...
                                   transition duration-200"
                        />
                        <button type="button" onclick="togglePwd('password_confirmation', 'eye-confirm')" class="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-200 transition">
                            <svg id="eye-confirm" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                <path stroke-linecap="round" stroke-linejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                            </svg>
                        </button>
            ...
            <p class="mt-6 text-center text-sm text-slate-400">
                ...
                <a href="{{ route('auth.login') }}" class="text-indigo-400 hover:text-indigo-300 font-medium transition">Sign in</a>
            </p>
        </div>
    </div>
</div>
